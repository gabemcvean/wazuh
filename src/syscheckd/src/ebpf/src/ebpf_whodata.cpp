                                    ...
                                    const char* detail = nullptr)
{
    ...
    if (detail)
    {
        snprintf(log_message, sizeof(log_message), format, action, detail);
    }
    else
    {
        snprintf(log_message, sizeof(log_message), format, action);
    }
...
static bool modify_healthcheck_file_metadata(const char* file_path, char* detail, size_t detail_size)
{
    ...
    {
        snprintf(detail, detail_size, FIM_EBPF_HEALTHCHECK_CHMOD_DETAIL, file_path, strerror(errno));
        return false;
    }
                                   ...
                                   bool enabled)
{
    ...
    if (!enabled)
    {
        log_healthcheck_message(LOG_ERROR, FIM_ERROR_EBPF_HEALTHCHECK_ACTION_SKIPPED, action,
...
int init_bpfobj()
{
    ...
    bpf_object__for_each_program(bpf_helpers, prog, obj)
    {
        ...
        struct bpf_link* link = bpf_helpers->bpf_program_attach(prog);
